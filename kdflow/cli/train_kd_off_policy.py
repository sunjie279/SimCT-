...
def train(args):
    ...
    if not tokenizer_info.vocab_identical and args.kd.kd_algorithm not in  ["dskd", "simple_ctkd", "alm", "uld", "span_ctkd"]:
        raise ValueError("Student and teacher tokenizers are not identical. Please use a cross-tokenizer KD algorithm (dskd, simple_ctkd, alm, uld, span_ctkd) or ensure tokenizers are the same.")
