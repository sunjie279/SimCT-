...
class FSDP2Strategy(ABC):
    ...
    def _fsdp2_load_full_state_dict(self, model, full_state, device_mesh, cpu_offload):
        ...
        if is_cpu_offload:
            # FSDP2 with CPUOffloadPolicy requires ALL managed parameters to live on CPU.
            # model.to("cpu") may not cover every DTensor shard (esp. root-module non-sharded
            # params like embed_tokens/norm when tie_word_embeddings=True). Move each
            # parameter's underlying local tensor explicitly to CPU.
            for _name, p in model.named_parameters():
                if hasattr(p, "_local_tensor"):
                    # DTensor (FSDP-managed sharded parameter)
                    p._local_tensor = p._local_tensor.to("cpu", non_blocking=True)
                if p.data.device.type != "cpu":
                    p.data = p.data.to("cpu", non_blocking=True)
            # Buffers should stay on GPU for forward computation
    ...
    def save_model(self, model: nn.Module, tokenizer, output_dir, **kwargs) -> None:
        ...
        self.log(f"Starting checkpoint save to {output_dir} ...")

        try:
            options = StateDictOptions(full_state_dict=True, cpu_offload=True)
            state_dict = get_model_state_dict(model_to_save, options=options)
        except Exception as e:
            self.log(f"ERROR: get_model_state_dict failed: {e}\n{traceback.format_exc()}", level="error")
            raise

        # Convert state_dict to bf16 to save storage space and loading time
        if self.bf16:
            state_dict = {k: v.to(torch.bfloat16) if v.is_floating_point() else v for k, v in state_dict.items()}
            self.log("State dict converted to bfloat16 for saving.")
            ...
            if self.is_rank_0():
                ...
                for fname in saved_files:
                    ...
                    if os.path.isfile(fpath):
                        ...
                        total_size += fsize
                        if fsize == 0:
                            self.log(f"WARNING: Saved file is empty: {fpath}", level="warning")

                self.log(f"Checkpoint saved successfully to {output_dir} "
                         f"({len(saved_files)} files, total {total_size / 1024 / 1024:.1f} MB)")
        except Exception as e:
            self.log(f"ERROR: save_pretrained failed: {e}\n{traceback.format_exc()}", level="error")
            raise
