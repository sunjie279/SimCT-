...
# Disable proxy for all local/internal HTTP requests to SGLang servers
    ...
    def _wait_server_healthy(base_url: str, is_process_alive, timeout: float = 600.0):
        ...
        with requests.Session() as session:
            while True:
                ...
                try:
                    response = session.get(f"{base_url}/health_generate", proxies=_NO_PROXY)
                    if response.status_code == 200:
                        break
            ...
            while True:
                ...
                try:
                    response = session.get(f"{base_url}/flush_cache", proxies=_NO_PROXY)
                    if response.status_code == 200:
                        break
                except requests.RequestException:
                    pass
